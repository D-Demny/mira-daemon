...
// (issue #17) playlistURIsInText must stop at a clean token boundary: protobuf
// dealer payloads carry binary tail bytes right after the literal token (T23
// build-152 capture). Only [0-9A-Za-z] runs of 20..25 chars yield candidates;
// everything else is rejected or trimmed away.
